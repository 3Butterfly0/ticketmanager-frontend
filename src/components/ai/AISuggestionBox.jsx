...
const AISuggestionBox = ({ title, description, onSuggestion }) => {
  ...
  return (
    <div className="bg-brand-tertiary/10 backdrop-blur-xl border border-brand-tertiary/20 rounded-3xl p-8 my-6 relative group transform transition-all duration-300">
        ...
        <div className="mt-6 space-y-5 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 bg-surface-card rounded-xl border border-brand-tertiary/20 shadow-sm">
              <span className="block text-[10px] text-text-muted font-bold uppercase mb-1 font-body">Category</span>
              <span className="font-bold text-text-main font-body">{suggestion.category}</span>
            </div>
            <div className="p-4 bg-surface-card rounded-xl border border-brand-tertiary/20 shadow-sm">
              <span className="block text-[10px] text-text-muted font-bold uppercase mb-1 font-body">Priority</span>
              <span className="font-bold text-text-main font-body">{suggestion.priority}</span>
            </div>
          </div>
          <div className="p-4 bg-surface-card rounded-xl border border-brand-tertiary/20 shadow-sm">
            <span className="block text-[10px] text-text-muted font-bold uppercase mb-1 font-body">Generated Summary</span>
            <p className="text-sm font-body text-text-main/80 leading-relaxed">{suggestion.summary}</p>
          </div>
          <button
      ...
      {!suggestion && !loading && !error && (
        <p className="text-sm text-indigo-900 opacity-60 italic">
          Fill in the title and description to get intelligent suggestions for category and priority.
        </p>
      )}
    </div>
  );
